...
class register extends CI_Controller {
    ...
    public function index(){
        ...
        if($enable_register == 0){
            $data['CONTENT'] = "msg";
            $data['MSG'] = $this->lang->line('register_closed_msg');
            ...
            if($_POST){
                ...
                if($captcha == $this->session->userdata('captcha')){
                    ...
                    if($password == $repassword){
                        if($this->__check('username', $this->input->post('username',true)) && $this->__check('email', $this->input->post('email',true))){
                            ...
                            if($active_register->value == 0)
                                $data['MSG'] = $this->lang->line('register_activation_msg');
                            else
                                $data['MSG'] = $this->lang->line('register_success_msg');
                        }else{
                            $vals = array(
                                    'img_path' => './captcha/',
                            ...
                            $data['ERROR'] = True;
                            $data['ERR_MSG'] = $this->lang->line('register_error_already_register_msg').anchor(base_url()."login/resetpassword",$this->lang->line('register_used_reset_password'));
                        ...
                        $data['CAPTCHA'] = $cap['image'];
                        $data['ERROR'] = True;
                        $data['ERR_MSG'] = $this->lang->line('register_error_password');
                    ...
                    $data['CAPTCHA'] = $cap['image'];
                    $data['ERROR'] = True;
                    $data['ERR_MSG'] = $this->lang->line('register_error_captcha');
        ...
        $data['TITLE'] = " -- ".$this->lang->line('register_index');
